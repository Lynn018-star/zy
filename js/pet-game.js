...
// 修复：每种宠物类型使用各自第一个品种的头像（替代原来全部指向同一图片的 bug）
Object.keys(PET_TYPES).forEach(function(type) {
    if (PET_BREEDS[type] && PET_BREEDS[type][0]) {
        PET_TYPES[type].img = PET_BREEDS[type][0].img;
    }
});
...
function showHouse() {
    ...
    // 生成家具列表（同时显示基础家具和已购买家具）
    const baseFurniture = Object.values(HOUSE_FURNITURE);
    const purchasedFurniture = (gameState.houseFurniture || []).map(function(f) {
        return { id: f.id, icon: f.icon, name: f.name, desc: f.desc, state: '已摆放' };
    });
    const furnitureList = baseFurniture.concat(purchasedFurniture);
...
window.interactFurniture = function(id) {
    ...
    // 同时从基础家具和已购买家具中查找
    let furniture = HOUSE_FURNITURE[id];
    if (!furniture) {
        furniture = (gameState.houseFurniture || []).find(f => f.id === id);
    }
    if (!furniture) return;
    const interactions = {
